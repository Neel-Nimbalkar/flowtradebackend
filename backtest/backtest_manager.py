...
class BacktestManager:
    ...
    def _run_job(self, job_id: str):
        ...
        try:
            ...
            # ✅ FIX: Use unified executor when connections are available (proper block ordering)
            connections = req.get('connections', [])
            
            if connections and len(connections) > 0:
                # Use unified executor with topological sorting
                from backendapi.workflows.unified_executor import execute_unified_workflow
                
                def execute_fn(workflow, latest):
                    """Execute workflow using unified executor with connections"""
                    # Build market_data for unified executor
                    market_data = {
                        ...
                        'close_history': latest.get('close_history', []),
                        'volume_history': latest.get('volume_history', []),
                        'high_history': latest.get('high_history', []),
                        'low_history': latest.get('low_history', [])
                    }
                    ...
                    signal, debug = execute_unified_workflow(
                        ...
                        debug=False  # Disable debug for performance in backtesting
                    )
                    
                    # Return a result object compatible with backtest_core expectations
                    class UnifiedResult:
                        def __init__(self, sig):
                            self.success = sig is not None
                            self.final_decision = sig
                    
                    return UnifiedResult(signal)
            else:
                # Fallback to old engine for legacy workflows without connections
